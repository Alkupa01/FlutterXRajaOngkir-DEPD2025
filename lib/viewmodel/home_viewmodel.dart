...
class HomeViewModel with ChangeNotifier {
  ...
  ApiResponse<List<City>> internationalDestList = ApiResponse.notStarted();
  setInternationalDestList(ApiResponse<List<City>> response) {
    internationalDestList = response;
    notifyListeners();
  }

  Future getInternationalDestList({String search = ''}) async {
    setInternationalDestList(ApiResponse.loading());
    _homeRepo
        .fetchInternationalDestination(search: search)
        .then((value) {
          setInternationalDestList(ApiResponse.completed(value));
        })
        .onError((error, _) {
          setInternationalDestList(ApiResponse.error(error.toString()));
        });
  }

  // International origin list (same API, different state)
  ApiResponse<List<City>> originInternationalList = ApiResponse.notStarted();
  setOriginInternationalList(ApiResponse<List<City>> response) {
    originInternationalList = response;
    notifyListeners();
  }

  Future getOriginInternationalList({String search = ''}) async {
    setOriginInternationalList(ApiResponse.loading());
    _homeRepo
        .fetchInternationalDestination(search: search)
        .then((value) {
          setOriginInternationalList(ApiResponse.completed(value));
        })
        .onError((error, _) {
          setOriginInternationalList(ApiResponse.error(error.toString()));
        });
  }

  // International cost list

...
class HomeRepository {
  ...
  // Mengambil daftar negara tujuan internasional dengan search
  Future<List<City>> fetchInternationalDestination({String search = ''}) async {
    final endpoint = 'destination/international-destination';

    // API requires search parameter, use space to get all results if empty
    final searchQuery = search.isEmpty ? ' ' : search;
    ...
    // Parse data negara - transform country_id/country_name to id/name
    final data = response['data'];
    if (data is! List) return [];

    return data.map((e) {
      // Transform international destination format to City format
      return City.fromJson({
        'id': int.tryParse(e['country_id'].toString()),
        'name': e['country_name'],
      });
    }).toList();
  }
